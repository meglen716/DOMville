...
// --- FIRE & PLAGUE CONSTANTS ---
const FIRE_CHANCE_BASE = 0.00002;   
const FIRE_GROWTH_TICK = 900;       
const FIRE_BURN_DOWN_TICK = 1800;   
const SPREAD_CHANCE = 0.003;        

const PLAGUE_INCUBATION_TICK = 3600; 
const PLAGUE_SPREAD_CHANCE = 0.03;   
...
const EARTHQUAKE_CHANCE = 0.000005; 

// Riot Variables
let highTaxTimer = 0;
let isRioting = false;
const RIOT_THRESHOLD = 1800; 
...
function manageDisasters(entities, gridSize, cars) {
    ...
    entities.forEach(ent => {
        ...
        // If an ambulance never arrives, the patient dies and the house abandons.
        if (ent.hasEmergency && !ent.isAbandoned && !ent.isBurned) {
            if (ent.emergencyTimer === undefined || ent.emergencyTimer <= 0) {
                ent.emergencyTimer = 2700; // Roughly 45 seconds of real-time
            }
            ent.emergencyTimer--;
            ...
            if (ent.emergencyTimer <= 0) {
                ...
                ent.color = '#bdc3c7'; // Grey out the house
                        ...
                        if (neighbor.type === 'house' && neighbor !== ent && !neighbor.isAbandoned && !neighbor.isBurned && !neighbor.hasEmergency) {
                            ...
                            if (dist <= gridSize * 3) { 
                                if (Math.random() < PLAGUE_SPREAD_CHANCE) {
                                    ...
                                    neighbor.emergencyTimer = 1800; // Shorter timer for plague
        ...
        // B. FIRE LOGIC (Healthy, Flammable Buildings)
    ...
    if (typeof currentTaxRate !== 'undefined') {
        if (currentTaxRate >= 1.5) {
            ...
            if (highTaxTimer === 0) isRioting = false;
            stopSFX('riot');
        }
    }
    ...
    let tornadoChance = isStorming ? 0.002 : 0.00002; 
    ...
    if (earthquakeTimer === 0 && Math.random() < EARTHQUAKE_CHANCE) { 
        earthquakeTimer = 400; 
        playSFX('earthquake', 0.8);
        earthquakeMagnitude = Math.random() * 10 + 8; 
        if (typeof logActivity === 'function') logActivity("EARTHQUAKE DETECTED! Brace yourself!", "emergency");
    }

    if (earthquakeTimer > 0) {
        earthquakeTimer--;
        if (earthquakeTimer % 30 === 0) {
            let activeBldgs = entities.filter(e => !e.isBurned && e.type !== 'park' && e.type !== 'waterPump');
            if (activeBldgs.length > 0) { let target = activeBldgs[Math.floor(Math.random() * activeBldgs.length)]; target.isBurned = true; target.fireLevel = 0; target.color = '#34495e'; if (typeof spawnDustParticles === 'function') spawnDustParticles(target.x, target.y, 40, '#2c3e50', gridSize); for (let i = cars.length - 1; i >= 0; i--) { if (cars[i].home === target) cars.splice(i, 1); } }
            
            let unburned = entities.filter(e => !e.fireLevel && !e.isBurned && ['house', 'office', 'supermarket', 'school', 'factory'].includes(e.type));
            if (unburned.length > 0) { let target = unburned[Math.floor(Math.random() * unburned.length)]; target.fireLevel = 1; target.fireTimer = 0; if (typeof spawnDustParticles === 'function') spawnDustParticles(target.x, target.y, 20, '#e67e22', gridSize); }

            if (typeof roads !== 'undefined' && roads.length > 0) {
                let roadIndex = Math.floor(Math.random() * roads.length); let path = roads[roadIndex];
                if (path.length > 1) {
                    let nodeIndex = Math.floor(Math.random() * path.length); let removedNode = path.splice(nodeIndex, 1)[0];
                    if (typeof spawnDustParticles === 'function') spawnDustParticles(removedNode.x, removedNode.y, 30, '#505050', gridSize);
                    let path1 = path.slice(0, nodeIndex); let path2 = path.slice(nodeIndex); roads.splice(roadIndex, 1);
                    if (path1.length > 0) roads.push(path1); if (path2.length > 0) roads.push(path2);
                } else { let removedNode = roads.splice(roadIndex, 1)[0][0]; if (typeof spawnDustParticles === 'function') spawnDustParticles(removedNode.x, removedNode.y, 30, '#505050', gridSize); }
            }
        }
    }
}
...
function updateAndDrawTornadoes(ctx, entities, cars, gridSize) {
    for (let i = tornadoes.length - 1; i >= 0; i--) {
        ...
        ctx.restore();
        if (t.x < -1000 || t.x > 5000 || t.y < -1000 || t.y > 5000) tornadoes.splice(i, 1);
        stopSFX('tornado');
    }
}